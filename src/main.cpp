...
#if defined(NDEBUG)
#error "GRVS cannot be compiled without assertions."
#endif
...
{
    //Check to see if the zGRVS is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zGRVS spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zGRVS without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zGRVS spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zGRVS spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zGRVS spend tx %s already in block %d",
            ...
            // Check that zGRVS mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight < 10080 && nHeight > 0) {
        nSubsidy = 0.1 * COIN;
    }
    else if (nHeight <= 11520 && nHeight >= 10080) {
        nSubsidy = 2 * COIN;
    }
    else if (nHeight <= 12960 && nHeight >= 11520) {
        nSubsidy = 3 * COIN;
    }
    else if (nHeight <= 14400 && nHeight >= 12960) {
        nSubsidy = 4 * COIN;
    }
    else if (nHeight <= 15840 && nHeight >= 14400) {
        nSubsidy = 5 * COIN;
    }
    else if (nHeight <= 17280 && nHeight >= 15840) {
        nSubsidy = 6 * COIN;
    }
    else if (nHeight <= 18720 && nHeight >= 17280) {
        nSubsidy = 7 * COIN;
    }
    else if (nHeight <= 20160 && nHeight >= 18720) {
        nSubsidy = 8 * COIN;
    }
    else if (nHeight <= 21600 && nHeight >= 20160) {
        nSubsidy = 9 * COIN;
    }
    else if (nHeight <= 23040 && nHeight >= 21600) {
        nSubsidy = 10 * COIN;
    }
    else if (nHeight <= 24480 && nHeight >= 23040) {
        nSubsidy = 11 * COIN;
    }
    else if (nHeight <= 25920 && nHeight >= 24480) {
        nSubsidy = 12 * COIN;
    }
    else if (nHeight <= 27360 && nHeight >= 25920) {
        nSubsidy = 13 * COIN;
    }
    else if (nHeight <= 28800 && nHeight >= 27360) {
        nSubsidy = 14 * COIN;
    }
    else if (nHeight <= 30240 && nHeight >= 28800) {
        nSubsidy = 15 * COIN;
    }
    else if (nHeight <= 31680 && nHeight >= 30240) {
        nSubsidy = 16 * COIN;
    }
    else if (nHeight <= 33120 && nHeight >= 31680) {
        nSubsidy = 17 * COIN;
    }
    else if (nHeight <= 34560 && nHeight >= 33120) {
        nSubsidy = 18 * COIN;
    }
    else if (nHeight <= 36000 && nHeight >= 34560) {
        nSubsidy = 19 * COIN;
    }
    else if (nHeight <= 37440 && nHeight >= 36000) {
        nSubsidy = 20 * COIN;
    }
    else if (nHeight <= 38880 && nHeight >= 37440) {
        nSubsidy = 21 * COIN;
    }
    else if (nHeight <= 40320 && nHeight >= 38880) {
        nSubsidy = 22 * COIN;
    }
    else if (nHeight <= 41760 && nHeight >= 40320) {
        nSubsidy = 23 * COIN;
    }
    else if (nHeight <= 43200 && nHeight >= 41760) {
        nSubsidy = 24 * COIN;
    }
    else if (nHeight <= 44640 && nHeight >= 43200) {
        nSubsidy = 25 * COIN;
    }
    else if (nHeight <= 46080 && nHeight >= 44640) {
        nSubsidy = 26 * COIN;
    }
    else if (nHeight <= 47520 && nHeight >= 46080) {
        nSubsidy = 27 * COIN;
    }
    else if (nHeight <= 48960 && nHeight >= 47520) {
        nSubsidy = 28 * COIN;
    }
    else if (nHeight <= 50400 && nHeight >= 48960) {
        nSubsidy = 29 * COIN;
    }
    else if (nHeight <= 60480 && nHeight >= 50400) {
        nSubsidy = 30 * COIN;
    }
    else if (nHeight <= 70560 && nHeight >= 60480) {
        nSubsidy = 20 * COIN;
    }
    else if (nHeight <= 80640 && nHeight >= 70560) {
        nSubsidy = 15 * COIN;
    }
    else if (nHeight <= 90720 && nHeight >= 80640) {
        nSubsidy = 10 * COIN;
    }
    else if (nHeight <= 416100 && nHeight >= 90720) {
        nSubsidy = 5 * COIN;
    }
    else {
        nSubsidy = 0.1 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZGRVSStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from GRVS
...
{
    RenameThread("grvs-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZGRVSMinted()
{
...
void RecalculateZGRVSSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zGRVS supply
...
bool RecalculateGRVSSupply(int nHeightStart)
{
...
bool UpdateZGRVSSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateGRVSSupply(Params().Zerocoin_StartHeight());
    }

    //Track zGRVS money supply in the block index
    if (!UpdateZGRVSSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zGRVS supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert GRVS to zGRVS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zGRVS spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zGRVS serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zGRVS stake block is less than allowed start height", __func__);

    if (CZPivStake* zGRVS = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zGRVS->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zGRVS stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zGRVS stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zGRVS->GetValue()));
        if (nChecksum200 != zGRVS->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zGRVS->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZGRVS() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zGRVS fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zGRVS mints and %d zGRVS spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // GRVS: We use certain sporks during IBD, so check to see if they are
