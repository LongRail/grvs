...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 GRVS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGRVS send
enum ZerocoinSpendStatus {
    ZGRVS_SPEND_OKAY = 0,                            // No error
    ZGRVS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGRVS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGRVS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGRVS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGRVS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGRVS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGRVS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGRVS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGRVS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGRVS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGRVS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGRVS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGRVS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGRVS_SPENT_USED_ZGRVS = 14,                      // Coin has already been spend
    ZGRVS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGRVS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZGRVSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzGRVSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGRVSTracker> zpivTracker;
    ...
    void setZWallet(CzGRVSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzGRVSTracker>(new CzGRVSTracker(strWalletFile));
    }

    CzGRVSWallet* getZWallet() { return zwalletMain; }
    ...
    /** zGRVS reset */
    boost::signals2::signal<void()> NotifyzGRVSReset;
