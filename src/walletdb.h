...
public:
    ...
    bool WriteZGRVSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGRVSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGRVSSeed_deprecated(uint256& seed);
    bool EraseZGRVSSeed();
    bool EraseZGRVSSeed_deprecated();

    bool WriteZGRVSCount(const uint32_t& nCount);
    bool ReadZGRVSCount(uint32_t& nCount);
