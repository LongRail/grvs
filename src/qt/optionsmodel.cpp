...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GRVS);
    ...
    if (!settings.contains("nAnonymizeGRVSAmount"))
        settings.setValue("nAnonymizeGRVSAmount", 1000);

    nAnonymizeGRVSAmount = settings.value("nAnonymizeGRVSAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGRVSAmount"))
        SoftSetArg("-anonymizegrvsamount", settings.value("nAnonymizeGRVSAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in grvs.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGRVSAmount:
            return QVariant(nAnonymizeGRVSAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGRVSAmount:
            nAnonymizeGRVSAmount = value.toInt();
            settings.setValue("nAnonymizeGRVSAmount", nAnonymizeGRVSAmount);
            emit anonymizeGRVSAmountChanged(nAnonymizeGRVSAmount);
            break;
        case CoinControlFeatures:
