...
CzGRVSWallet::CzGRVSWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGRVSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGRVSSeed_deprecated()) {
                    LogPrintf("%s: Updated zGRVS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzGRVSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGRVSCount(nCountLastUsed);
    else if (!walletdb.ReadZGRVSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGRVSWallet::Lock()
{
    seedMaster = 0;
}

void CzGRVSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGRVSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGRVS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGRVSWallet::LoadMintPoolFromDB()
{
...
void CzGRVSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGRVSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGRVSWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzGRVSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGRVS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzGRVSWallet::SeedToZGRVS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGRVSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGRVSWallet::GenerateDeterministicZGRVS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGRVSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGRVSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
