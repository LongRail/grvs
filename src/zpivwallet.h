...
#ifndef GRVS_ZGRVSWALLET_H
#define GRVS_ZGRVSWALLET_H
...
public:
    CzGRVSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGRVS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGRVS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GRVS_ZGRVSWALLET_H
