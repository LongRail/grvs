...
// The zGRVS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zGRVS is a hash of the serial
...
{
    //Create an output returning the zGRVS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZGRVSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zGRVS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zGRVS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZGRVSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zGRVS output", __func__);
...
{
    CzGRVSTracker* zpivTracker = pwallet->zpivTracker.get();
    CMintMeta meta;
...
{
    //The unique identifier for a GRVS stake is the outpoint
